	  ...
	  <div class="qi_home_shuai" v-show="screenDetail">
		 	...
		 	<div class="qi_home_shuai_con">
		 		...
		 		<div class="huoyuedu_con"> 
		 			<div class="item " :class="{'active':selectType===1}" @click="select(1,'新增')"><span>新增</span></div>
		 			<div class="item " :class="{'active':selectType===2}" @click="select(2,'活跃')"><span>活跃</span></div>
		 			<div class="item " :class="{'active':selectType===3}" @click="select(3,'沉睡')"><span>沉睡</span></div>
		 			<div class="item " :class="{'active':selectType===4}" @click="select(4,'流失')"><span>流失</span></div>
		 		</div>
		 		</div>
		 		<div class="zuijindao_wrap">
		 			...
		 			<div class="zuijindao_con">
		 				<div class="date_xuan">
		 					<div class="item" id="calendar" ><span>起始：</span><input type="text" placeholder="0000-00-00" readonly id="birthday" v-model="selectdianStart" ><img  src="../../assets/images/home_menu_date@2x.png"/><datetime  v-model="selectdianStart"   @on-confirm="fangonConfirm" class="fang"></datetime></div>
		 					
		 					<div class="item" id="calendar1" ><span>结束：</span><input type="text" placeholder="0000-00-00" readonly id="jieshu" v-model="selectdianEnd" ><img  src="../../assets/images/home_menu_date@2x.png"/><datetime  v-model="selectdianEnd"  @on-confirm="fangonConfirm" class="fang"></datetime></div>
		 					
		 				</div>
		 				<div class="shijian_duan" id="zuijindao">
		 					<div class="item" daodiandate="yizhou" :class="{'active':selectdianType===1}" @click="selectdian(1,'yizhou')"><span>最近一周</span></div>
		 					<div class="item" daodiandate="yiyue"  :class="{'active':selectdianType===2}" @click="selectdian(2,'yiyue')"><span>最近一月</span></div>
		 					<div class="item" daodiandate="sanyue"  :class="{'active':selectdianType===3}" @click="selectdian(3,'sanyue')"><span>最近三月</span></div>
		 					<div class="item" daodiandate="bannian"  :class="{'active':selectdianType===4}" @click="selectdian(4,'bannian')"><span>最近半年</span></div>
		 				</div>
		 			</div>
		 		</div>
		 		...
		 		<div class="zuijindao_wrap">
		 			...
		 			<div class="zuijindao_con">
		 				<div class="date_xuan">
		 					...
		 					<div class="item" ><span>起始：</span><input type="text"  placeholder="0000-00-00" readonly id="birthday1"  v-model="overCardStart"><img  src="../../assets/images/home_menu_date@2x.png"/><datetime  v-model="overCardStart"  @on-confirm="fangonConfirm" class="fang"></datetime></div>
		 					
		 					<div class="item" ><span>结束：</span><input type="text" placeholder="0000-00-00" readonly id="jieshu1"  v-model="overCardEnd" ><img  src="../../assets/images/home_menu_date@2x.png"/><datetime  v-model="overCardEnd"  @on-confirm="fangonConfirm" class="fang"></datetime></div>
		 					
		 				</div>
		 				<div class="shijian_duan" id="jianshenguoqi">
		 					<div class="item " daodiandate="yizhou" :class="{'active':selectdian1Type1===1}" @click="selectdian1(1,'yizhou')"><span>最近一周</span></div>
		 					<div class="item" daodiandate="yiyue"  :class="{'active':selectdian1Type1===2}" @click="selectdian1(2,'yiyue')"><span>最近一月</span></div>
		 					<div class="item" daodiandate="sanyue"  :class="{'active':selectdian1Type1===3}" @click="selectdian1(3,'sanyue')"><span>最近三月</span></div>
		 					<div class="item" daodiandate="bannian"  :class="{'active':selectdian1Type1===4}" @click="selectdian1(4,'bannian')"><span>最近半年</span></div>
		 				</div>
		 			</div>
		 		</div>
		 		<div class="qi_sure_height"></div>
		 		<div class="qi_sure">
		 			<div class="item" id="congzhi" @click="congzhi()"><span>重置</span></div>
		 			<div class="item sure" id="suan_sure" @click="screennone()"><span>确定</span></div>
		 		</div>
		 	</div>
		 </div>
	 <mt-loadmore :top-method="loadTop"   :max-distance="150"
   	 ...
   	 <div class="qi_my_coustomer">
		    ...
		    <div class="qi_search_wrap">
		    	<div class="seach">
		    		<input type="text" id="search_val" placeholder="请输入姓名/卡号/手机号码查找会员" v-model="kehunametel">
		    		<span id="search_wrap" @click="keseachbtn()"><em>搜索</em></span>
		    	</div>
		        <div class="shuxian"><span class="line"></span></div>
		        <div class="shuaixuan" @click="screenShow()"><span :class="{shuaixu:fangscreen}">筛选</span>&nbsp&nbsp<em class="triangle_border_down" :class="{shuaixu1:fangscreen}"></em></div>
		    </div>
              <div
			class="datacontent"
		...
		>
		    <div class="mycustomer_list">		    	
				<div class="mycustomer_item"  v-for="(item1,index) in getmycustomerlist.length>0?getmycustomerlist:searchlist" :key="index" @click="glass(item1.fldclientid)">
				  <div class="mycustomer_fang">
        			<div class="yewu_ico"><img src="../../assets/images/default.png" /><div class="item" v-show="item1.sameMobileCount>1"><span v-show="item1.sameMobileCount<=99">{{item1.sameMobileCount}}</span><span  v-show="item1.sameMobileCount>99">99+</span></div></div>
					<div class="yewu_text yewu_text2">
						<div class="jianshen_text fangname">{{item1.fldfirstname}}</div>
						<div class="fangtel"><div class="itemtel">{{item1.fldworkphone}}</div></div>						
					</div>
				   <div class="yewu_sanjiao item" v-if="item1.firstRegistCard ==undefined && item1.lastVisit==undefined">
			...
			<!-- 筛选显示 -->
              <div class="mycustomer_list">		    	
				<div class="mycustomer_item"  v-for="(item1,index) in getshaixuanvalList.length>0?getshaixuanvalList:getshaiguoqiList" :key="index" @click="glass(item1.fldclientid)">
				  <div class="mycustomer_fang">
...
Promise.polyfill();
 export default {
  ...
  if(this.selectValue !="" || this.selectdianStart != "" || this.selectdianEnd != "" ||this.yearcishuStart != "" || this.yearcishuEnd != "" || this.yearjineStart != "" || this.yearjineEnd !="" || this.overCardStart != "" || this.overCardEnd !=""){
  ...
  if(this.selectValue =="" && this.selectdianStart == "" && this.selectdianEnd == "" &&this.yearcishuStart == "" && this.yearcishuEnd == "" && this.yearjineStart == "" && this.yearjineEnd ==""){
			 this.getgonghao();
			 return;
  }  
   ...
   getshaiguoqi(){
      ...
      this.instance.$post('/rest/wx/customerGx/findCustomerGxCardOver',{
        pageFlag:true,
		pageNo:this.pageVal,
		pageSize:this.pageSize,
...
.notScroll {
   ...
   .qi_home_header{width:100%;;background: #4978B0;position: relative;}	     
		  ...
		  .qi_search_wrap{padding:0 5%;height:1.04rem;}
	      ...
	      .seach input{width:76%;height:0.6rem;padding-left:0.1rem;outline:none;font-size: 0.3rem;border: 1px solid #D6D6D6;
	       border-radius: 4px 0 0 4px; -webkit-appearance: none; 
	      }
	      #search_wrap{display: inline-block;width:22%; height:0.64rem;margin-top:0.2rem;font-size: 0.3rem;color:#fff;background:#4a77b0;
	                  position: absolute;top:0;right:0;line-height: 0.64rem;text-align: center;border-radius: 0 4px 4px 0;
	      }
	      ...
	      .shuxian{width:7%;text-align: center;float: left;padding-top: 0.22rem;padding-left: 1%;}
	      .shuxian .line{display: inline-block;width:1px;height:0.6rem;background: #999;opacity: 0.6;}
		...
		.yewu_ico{width:11.4%;position: relative;box-sizing:border-box;}
		.yewu_sanjiao {
			float: left;
			padding-top: 0.2rem;
			width:38%;
    ...
    padding: 0 0.1rem;
		...
		border-top: 1px solid #f7f7f7;
		   ...
		   .bottom_wrap .item .active{background: #DCE4F1;}
		    ...
		    .bottom_wrap p{font-size: 0.2rem;color: #AFAFAF;padding-top: 0.1rem;}
		    .shouye{background: #DCE4F1;border-radius: 6px;}
		    .kehu{background: #DCE4F1;border-radius: 6px;}
		    .jingying{background: #DCE4F1;border-radius: 6px;}
		    .me{background: #DCE4F1;border-radius: 6px;}
		    .bottom_height{width:100%;height:1.2rem;}
		    /*点击帅选*/		    
			.qi_home_shuai{position: fixed;top:0;left:0;right: 0;bottom:0;background: rgba(0,0,0,0.2);z-index: 99999999;}
			.qi_home_shuai_con_left{width:20%;height:100%;background:rgba(0,0,0,0);position: absolute;left:0;overflow-y:auto}		    
		    .qi_home_shuai_con{width:78%;height:100%;background: #F7F7F7;position: absolute;right:0;overflow-x:hidden}  
		    
		    .huoyuedu_wrap{background: #fff;padding-left: 3%;}
		     .huoyuedu_wrap p{height:0.79rem;line-height:0.79rem;font-size:0.3rem ;color: #1D1D1D;border-bottom: 1px solid #D6D6D6;}
		    .huoyuedu_con{display: flex;height:0.9rem;padding-top:0.3rem;}
		    .huoyuedu_con .item{flex: 1;height:0.6rem;line-height:0.6rem; background: #F7F7F7;margin-right:3%;text-align: center;
		       font-size: 0.24rem;color:#1D1D1D;
		    }
		     .huoyuedu_con .item.active{background: #4978B0;color:#fff;}
		    .zuijindao_wrap{background: #fff;padding-left: 3%;margin-top: 0.3rem;}
		    .zuijindao_wrap p{height:0.79rem;line-height:0.79rem;font-size:0.3rem ;color: #1D1D1D;border-bottom: 1px solid #D6D6D6;}		  
		    .zuijindao_con{}
		   
		    .date_xuan{display: flex;margin-top:0.3rem;}
		    
			.date_xuan .item{flex:1;height:0.6rem; background:rgba(247,247,247,1);border-radius:2px;margin-right: 3%;position: relative;}
			.fang{width:100%;height:100%;position: absolute;top:0;left: 0;opacity: 0;}
		    .date_xuan span{font-size: 0.22rem;color:#1D1D1D;padding-left:4%;}
		     .date_xuan input{width:51%;height:0.4rem;border: 0;background: rgba(247,247,247,1);;
		     margin-right: 2%;padding-top:0.14rem;outline: none;
		     }
		     .date_xuan img{width:12%;height:0.36rem;position: relative;top:0.05rem;}
		     .shijian_duan{display: flex;padding:0.3rem 0; }
		     .shijian_duan .item{flex:1;height:0.6rem;line-height: 0.6rem;text-align: center; background:rgba(247,247,247,1);border-radius:2px;margin-right:3%;
		       font-size: 0.24rem;color:#1D1D1D;
		     }
			 .shijian_duan .item.active{background: #4978B0;color:#fff;}
		     .year_jine_wrap{background: #fff;padding-left: 3%;margin-top: 0.3rem;}
		      .year_jine_wrap p{height:0.79rem;line-height:0.79rem;font-size:0.3rem ;color: #1D1D1D;border-bottom: 1px solid #D6D6D6;}
		     .year_jine_con{display: flex;padding: 0.3rem 3% 0.3rem 0;}
		     .year_jine_con input{ flex:1;width:100%;height:0.6rem;outline: none;text-align: center;background:#F7F7F7;border: 0;}
		     .qujianxian{width:4%;height:2px;background: #1D1D1D;position: relative;top:0.28rem;}
