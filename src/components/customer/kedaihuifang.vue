	 ...
	 <!--回访筛选-->
		 <div class="qi_home_shuai" v-show="huishuaixuan1">
		 		...
		 		<div class="item" v-for="(item,index) in fangshuaixuanlist" :key=index>
		 			<div class="itemp">
						...
						<span class="check_wrap">
			              <input type="checkbox"  :value="item.dictionaryId" name="fangch" @click="selectshuai(index,$event)" v-model="checkedValue"/>
			              <span class="xuanzhong" v-show="(checkedValue.toString()).indexOf(item.dictionaryId)!=-1"></span>
		 	...
		 	<div class="qi_sure">
		 			...
		 			<div class="item sure" id="suan_sure" @click="suansure1()"><span>确定</span></div>
		 	</div>
		 </div>
      <mt-loadmore :top-method="loadTop"   :max-distance="150"
			...
			<div class="qi_hui_fang_header">
				<div class="daihuifang active">
					<span>等待回访</span>
				</div>
                
				<div class="huifanglishi" @click="huifanglish()">
					<span>回访历史</span>
				</div>
		...
		>
		      <div class="qi_hui_data_list0">
				<div class="qi_hui_fang_item"  v-for="(item,index) in getyuandailist" :key="index">
                   <div @click="dengjijieguo(item.visitId,item.customerGxName,item.customerGxMobile,item.visitPlanDate,item.visitTypeName,item.remarks)">
                        ...
                        <div class="qi_hui_data">
                            ...
                            <div class="fangvalue1">{{item.visitPlanDate?(item.visitPlanDate).split(' ')[0]:""}}</div>
                        </div>
                        <div class="qi_hui_data">
                        ...
                        <div class="qi_hui_data">
                            ...
                            <div class="quxiao_huifang" @click.stop="quxiaoshow(item.visitId)"><span>取消回访</span></div>
                        </div>
                   </div>
			    </div>
			...
			<div class="no-more" v-if="noMore">没有更多数据了</div> 
              ...
              <!-- 搜索展示      -->
               <div class="qi_hui_data_list0">
				<div class="qi_hui_fang_item"  v-for="(item,index) in seachhuilist.length>0?seachhuilist:shuairesultlist" :key="index">
                   <div @click="dengjijieguo(item.visitId,item.customerGxName,item.customerGxMobile,item.visitPlanDate,item.visitTypeName,item.remarks)">
                        <div class="qi_hui_data">
                            <div class="mingcheng"><img src="../../assets/images/icon_girl@2x.png" />&nbsp&nbsp<span>{{item.customerGxName}}</span></div>
                            <div class="fangvalue">&nbsp&nbsp&nbsp&nbsp<a :tel="item.customerGxMobile">{{item.customerGxMobile}}</a></div>
                        </div>
                        <div class="qi_hui_data">
                            <div class="mingcheng1">回访日期：</div>
                            <div class="fangvalue1">{{item.visitPlanDate?(item.visitPlanDate).split(' ')[0]:""}}</div>
			     ...
			     <div class="zanwujilu" v-show="zanwushow">
	   ...
	   <div class="tianjia" @click="tianjiahuifang1()"></div>               
 </div>
</template>
