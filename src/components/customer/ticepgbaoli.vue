 ...
 <div class='container'>
   <div>
    <div class="tepg_header" v-if="heightflag">
     ...
     <div class="fengeline"></div><!--分割线 -->
     <!-- 体态动态评估 -->
      <div class="dongpg_wrap"  v-show="!nodongflag">
 ...
 export default {
   ...
   methods: {
    ...
    previewImage(imgurl){
     ...
     //体态静态评估
     getjtpg(){
       this.instance.$post('/rest/wx/reception/getStaticEvaluation',{
           mobile:this.mobile
       },res => {        
         console.log('静态')
        console.log(res);
        if(res.status){
           var data1 = res.rows[0]; 
            if(data1){              
            console.log(data1)
