...
<div class="weui-cells weui-cells_form">
  <div class="weui-cell">
    ...
    <div class="weui-cell__bd">
      <input class="weui-input" id="name" type="text" maxlength="50" placeholder="请输入客户姓名" readonly="readonly" v-model="cusname">
    </div>
  </div>
  <div class="weui-cell">
   ...
   <!-- <div class="weui-cell">
    ...
    <div class="weui-cell__bd arrow">
      <input class="weui-input painput" @click="suretype()" v-model="currentTags"  id="huifang_type" type="text" readonly="readonly" placeholder="请选择回访类型">
    </div>
  </div> -->
  <popup-picker :title="title1" :data="lists" v-model="value"  @on-change="onChange" :placeholder="('请选择回访类型')"></popup-picker>
   <div class="weui-cell" id="showDatePicker">
    ...
    <div class="weui-cell__bd arrow">
      <input class="weui-input painput1"  id="dateBirth" type="text" placeholder="请选择回访日期"  :value="pickerValue"  readonly="readonly"  @click="openPicker()">
    </div>
  </div>
 ...
 <div class="addCustomer_finish" id="huifang_save" @click="savehuifang()"><span>保存</span></div>
  ...
  <!-- <div class="picker_wrap" v-show="showpicer">
...
import { PopupPicker } from 'vux'
 export default {
   ...
   methods: {
       ...
       console.log(res);
        var data = res.rows[0];       
       this.cusname =res.rows[0].fldfirstname;
       this.cusphone =res.rows[0].fldworkphone;
   ...
   mounted(){
    $('.painput').css({"position":"relative","right":"14px"})
     $('.painput1').css({"position":"relative","right":"14px"})
   },
   updated(){
     $('.painput').css({"position":"relative","right":"14px"})
     $('.painput1').css({"position":"relative","right":"14px"})
...
//  .picker_wrap{position: absolute;top:0;left:0;top:0;right:0; background: rgba(0,0,0,0.5)}
...
.qi_search_wrap{padding:0 5%;height:1rem;margin-bottom: 0.3rem;}
  ...
  #search_wrap{display: inline-block;width:22%; height:0.6rem;margin-top:0.2rem;font-size: 0.3rem;color:#fff;background:#4a77b0;
              position: absolute;top:0;right:0;line-height: 0.6rem;text-align: center;border-radius: 0 10px 10px 0;
  }
