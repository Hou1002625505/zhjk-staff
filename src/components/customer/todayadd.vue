 ...
 <div class='container'>
     <div class="headerbg" v-show="!zanwuflag"></div>
      <div class="mycustomer_list">		    	
				<div class="mycustomer_item"  v-for="(item1,index) in addList" :key="index" @click="glass(item1.customerId)">
				  <div class="mycustomer_fang">
        			<div class="yewu_ico"><img src="../../assets/images/default.png" /></div>
					<div class="yewu_text yewu_text2">
						<div class="jianshen_text fangname">{{item1.name}}</div>
						<div class="fangtel"><div class="itemtel">{{item1.mobile}}</div></div>						
					</div>
				  </div>
               </div>     
	 </div> 
...
import { Datetime } from 'vux'
 export default {
   ...
   methods: {
     getaddlist(){
       this.instance.$post('/rest/wx/employee/getAddCustomerListByEmp',{},res => {
          console.log(res);
          if(res.status){
              var data = res.rows;
              data.forEach(item => {
