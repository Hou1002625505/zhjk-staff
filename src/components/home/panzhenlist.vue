 ...
 <div class='container'>
   ...
   <div class="item_wrap">			
		<div class="item_list_wrap">	
			<div class="item_list" v-for="(item,index) in getcusinfoList" :key="index" @click="pandetail(item.diagnosisId,item.dictionaryName,item.cusItemId,index)">
				<div class="itemType">项目类型:{{item.dictionaryName}}<span class="compile" v-show="index==0">编辑</span></div>
                ...
                <div class="itemDetailed">
					<span>操作人</span>
					<span>{{item.subhealthWrite}}</span>
				</div>
                <div class="itemDetailed">
					<span>顾问</span>
					<span>{{item.customerPlanWrite}}</span>
				</div>
				
				<div class="itemDetailed">
					<span>填表时间</span>
					<span>{{filDate(item.writeTime)}}</span>
				</div>
			</div>
...
<script>
 export default {
   ...
   methods: {
     getcusinfo(){
        this.instance.$post('/rest/wx/diagnosis/getDiagnosislListPage',{
            page:this.pageVal,
			rows:this.pageSize,
			param:{
				id:this.customerId
     ...
     pandetail(id,itemName,itemid,index){
         console.log(index)
         if(index==0){
            this.$router.push({
                name: "panzhenbian",
