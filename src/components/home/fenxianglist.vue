    ...
    >
      ...
      >
        <div class="card_g">
          <div class="card_list">
            <div
              v-for="(item,index) in getcarlist"
              :key="index"
              @click="qufenxiang(item.couponId,item.packageName)"
            >
              <div :class="{coupons:item.couponType==1,experience:item.couponType==0}">
                <div class="card_top">
                ...
                <div class="card_bottom">
                  <div class="date">领取日期截止：{{formatDate(item.validDateEnd)}}</div>
...
export default {
  ...
  methods: {
    ...
    getcarlistvalue() {
      this.instance.$post(
        "/rest/wx/coupon/getEmpShareList",
        {
          page: this.pageVal,
          rows: this.pageSize,
          param: {
