<template>
 <div class='container' v-wechat-title="ftitle">
   <mt-swipe :show-indicators="false" @change="handleChange" :continuous="false" :auto="0" :defaultIndex="swiperItemIndex">
      <mt-swipe-item class="swiper-demo-img" v-for="(item, index8) in weekList" :key="index8" >
     <div class="dataList">
         <ul>
           <li class="zdata" v-for="(item,index) in item.items" :key="index" @click="selectclitime(item)">
                <p>{{item.substring(0,5)}}</p>
             <p style="position:relative;top:-4px;">{{item.substring(0,5)==currentTime.replace(/-/g,'.').substring(5,10)?'今天':item.substring(6,8)}}</p>
  ...
  <div class="coursetable">
         <div class="coursetime clear">
             <div class="courseitem" v-for="(item,index1) in times"  :key="index1" @click="yiyuyue.indexOf(item.xianshitime)>=0&&fyijiesuarr.indexOf(item.xianshitime)<0&&clickyiyuyue(item.xianshitime)"  :class="{courseitemactive:'',bottomright:item.id==35,bottomleft:item.xianshitime=='20:00'}">
                <span :class="{active:yifabucourse.indexOf(item.xianshitime)>=0,active1:yiyuyue.indexOf(item.xianshitime)>=0&&fyijiesuarr.indexOf(item.xianshitime)<0}" v-html="yiyuyue.indexOf(item.xianshitime)>=0?fyijiesuarr.indexOf(item.xianshitime)>=0?'已结束':'已约':item.xianshitime">{{item.xianshitime}}</span>
...
<div v-show="fxiajiaflag">
   <div class="coursetable">
         <div class="coursetime clear">
             <div class="courseitem" v-for="(item,index1) in times"  :key="index1" @click="yifabucourse.indexOf(item.xianshitime)>=0&&selectxiajiame(item.id)" :class="{courseitemactive:'',bottomright:item.id==35,bottomleft:item.xianshitime=='20:00'}">
                <span :class="{active:yifabucoursexiajia.indexOf(item.id)>=0||yifabucourse.indexOf(item.xianshitime)>=0,active1:yiyuyue.indexOf(item.xianshitime)>=0&&fyijiesuarr.indexOf(item.xianshitime)<0}" v-html="yiyuyue.indexOf(item.xianshitime)>=0?fyijiesuarr.indexOf(item.xianshitime)>=0?'已结束':'已约':item.xianshitime">{{item.xianshitime}}</span>
               <img :src="yifabucoursexiajia.indexOf(item.id)>=0?checkbtn1:checkbtn" v-if="item.xianshitime!=''&&yiyuyue.indexOf(item.xianshitime)<0&&yifabucourse.indexOf(item.xianshitime)>=0"/>
             </div>
  ...
  <div class="xiajiayuanyin">
     ...
     <div class="yuanyinitem_wrap clear">
          <div class="yuanyinitem" v-for="(item,index3) in xiajiareasonlist" :key="index3" :xiajiayyId="item.dictionaryId" :class="{active:item.dictionaryId==selectxiajiaid}" @click="selecxiajiame(item.dictionaryId,item.name)">
             {{item.name}}
         </div>
...
<div v-show="fzhuijiaflag">
   <div class="coursetable">
         <div class="coursetime clear">
             <div class="courseitem" v-for="(item,index1) in times"  :key="index1" @click="yifabucourse.indexOf(item.xianshitime)<0&&selectedzhuijia(item.id)" :class="{courseitemactive:'',bottomright:item.id==35,bottomleft:item.xianshitime=='20:00'}">
                <span :class="{active:yifabucoursezhuijia.indexOf(item.id)>=0,active1:yiyuyue.indexOf(item.xianshitime)>=0&&fyijiesuarr.indexOf(item.xianshitime)<0}" v-html="yiyuyue.indexOf(item.xianshitime)>=0?fyijiesuarr.indexOf(item.xianshitime)>=0?'已结束':'已约':item.xianshitime">{{item.xianshitime}}</span>
               <img :src="yifabucoursezhuijia.indexOf(item.id)>=0?checkbtn:checkbtn1" v-if="item.xianshitime!=''&&yiyuyue.indexOf(item.xianshitime)<0"/>
             </div>
 ...
 export default {
   data () {
     ...
     weekList1:[],
      ...
      circleselected:this.config.formatDate(this.config.currentTime()).replace(/-/g,'.').substring(5,10),
      fnodataflag:false,
      courselist:false,
      yifabucourse:[],
      yifabucourseid:[],
     ...
     customerId:'',
            ...
            customerName:'',
              ...
              classtype1:'',
               ...
               orderType:'',
               no:'',
               url1:this.config.service_ip1+'/train/?v='+new Date().getTime()+'#/management/my_management',//我的预约
     }
   },
   created(){
    ...
    // }
    for (var i = 0; i < 7; i++) {
          var week =['周日','周一','周二','周三','周四','周五','周六'][new Date(new Date(timesStamp + 24 * 60 * 60 * 1000 * (i - (currenDay + 6) % 7))).getDay()]; 
      // console.log(week) 
      var date='',date1='';
      // console.log(this.config.formatDate(timesStamp + 24 * 60 * 60 * 1000 * (i - (currenDay + 6) % 7)).split('-')[1]);
      var shds=this.config.formatDate(timesStamp + 24 * 60 * 60 * 1000 * (i - (currenDay + 6) % 7)).split('-')[1];
      var shds1=this.config.formatDate(timesStamp + 24 * 60 * 60 * 1000 * (i - (currenDay + 6) % 7)).split('-')[2];
      date=shds+"."+shds1;
      // console.log(date)
        dates.push(date+"."+week);
         date1=this.config.formatDate(timesStamp + 24 * 60 * 60 * 1000 * (i - (currenDay + 6) % 7)).split('-')[0]+"-"+shds+"-"+shds1;
   ...
   methods: {
     ...
     querycourse(){
        ...
        //   console.log(timeList)
         this.instance.$post('/rest/wx/coach/course/getCourseTimeListOfCoach',{
     ...
     surecoursexiajia(){
        ...
        var yidata = this.yiyuyue;
          ...
          //已预约id + 已发布id
          var yiyuyuefabuid = this.unique(yiyuyueid.concat(this.yifabucoursexiajia))
      ...
      surecoursezhuijia(){
        ...
        var yidata = this.yiyuyue;
          ...
          //已预约id + 已发布id
        var yiyuyuefabuid = this.unique(yiyuyueid.concat(this.yifabucoursezhuijia))
        var xiajiayixuan = this.bubbleSort(yiyuyuefabuid)
           console.log(xiajiayixuan)
          var shijianduan = [];
         var ftimes = this.times;
        ...
        shijianduanstr = shijianduan1.join(',')
         ...
         this.instance.$post('/rest/wx/coach/course/editCourseTime',{
                coachCode:this.userName,
                coachCourseTimeId:this.courseid,            
                courseTimeScopes:shijianduanstr,
                xiajiayyId:'123',
     ...
     clickyiyuyue(fshijianduan1){
      ...
      this._fffgetSingerList()
       ...
       this.instance.$post("/rest/wx/coach/getOrderListPageByCoach",data,res => {
                ...
                if(data.length>0){
                    ...
                    for(var i = 0;i<data.length;i++){
                       ...
                       if(this.gethourcha(timeScope1,timeScope2,this.ffshijianduan1)){
                     ...
                     window.location.href = this.url1+'?customerId='+this.customerId+'&customerName='+this.customerName+'&headImg='+this.headImg+'&no='+this.no+'&appointDate='+this.appointDate+'&timeScope='+this.timeScope+'&tel='+this.tel+'&clubName='+this.clubName+'&courseType='+this.courseType+'&tradePackage='+this.tradePackage+'&fitName='+this.fitName+'&number='+this.number+'&itemName='+this.itemName+'&deductions='+this.deductions+'&deductionsZz='+this.deductionsZz+'&remainNum='+this.remainNum+'&remainNumZz='+this.remainNumZz+'&No='+this.no+'&plandetailId='+this.plandetailid+'&planDetailName='+this.planDetailName+'&status='+this.status+'&packageId='+this.packageId+'&sysClubCode='+this.sysClubCode+'&itemId='+this.itemId+'&deductionsBz='+this.deductionsBz+'&relaId='+this.relaId+'&orderType='+this.orderType
...
if(this.weekList.length-1==index){
 ...
 //this._getSingerList(0,0);
  ...
  //  this._getSingerList(0,0);
        ...
        this.month=parseInt(this.weekList[this.swiperItemIndex].items[0].substring(0,2));
     ...
     selectclitime(dateid){
       ...
       var shacha = new Date(selectdate).getTime()-new Date(this.currentTime1).getTime();
      //  alert(shacha)
       if(shacha<0){
      ...
      if(dates.length-1==index){
       ...
       this.weekList.forEach((element,index) => {
         element.items.forEach((element,index1) => {
            if(element.substring(0,5)==this.currentTime.replace(/-/g,'.').substring(5,10)){
       ...
       this.instance.$post("/rest/wx/coach/getOrderListPageByCoach",data,res => {
             ...
             if(data.length>0){
               data.forEach(item => {
                  ...
                  //课程结束，已扣费私教自己约的
                   if(item.status==2||(item.status==7&&item.orderType==0)){
...
//数组去重
  ...
  switchs(){
   this.$router.replace({
    ...
    goDetails(customername,name1,appointdate,timescope,tell,clubname,coursetype,tradepackage,fitname,numbers,itemname,deductionS,deductionszz,remainnum,remainnumZz,no,plandetailid,planDetailname,statuss,customerid,headimg,No,packageid,sysclubcode,itemid,deductionsbz,relaid){
...
<style scoped lang="less">
    ...
    .mint-swipe{ height:74px!important;}
    .menus{
            max-width: 640px;
              width:96%;
    position: fixed;
...
color:rgba(153,153,153,1);}
         ...
         .active{color:#1D1D1D!important;}
         .active1{color:#4978B0!important;}
         img{width:0.28rem;position: absolute;top:0;right: 0;}
        }
...
font-weight:500;
color:rgba(153,153,153,1);}
.cursorbianji{padding:0 3%;height:0.6rem;}
.cursorbianjiitem{float: right;}
.cursorbianji span{
...
font-weight:500;
color:rgba(153,153,153,1);}
.xiajiayuanyin{padding-right: 3%}
.xiajiayuanyin p{padding-left: 3%;font-size:0.3rem;
font-family:PingFang-SC-Medium;line-height: 0.9rem;
font-weight:500;
color:rgba(29,29,29,1);}
.yuanyinitem_wrap{}
.yuanyinitem{width:46.4%;margin:0 0 0.4rem 3%;
height:0.6rem;float: left;
line-height: 0.6rem;
text-align: center;
...
.yuanyinitem_wrap .active{background:rgba(73,120,176,1)!important;color:#FFFFFF!important;}
.textarea_wrap{width:94%;margin:0 auto;
height:1.5rem;
background:rgba(247,247,247,1);}
.textarea_wrap textarea{width:96%;height:96%;padding:3%; resize: none;background:rgba(247,247,247,1);border: none;font-size:0.26rem;
font-family:PingFang-SC-Medium;outline: none;
font-weight:500;
color:rgba(29,29,29,1);}
.fcoursefabubtn{width:47%;float: left;
height:0.8rem;
line-height: 0.8rem;
text-align: center;
border:1px solid rgba(73,120,176,1);
...
.fcoursefabubtn1{width:47%;float: right;
height:0.8rem;
line-height: 0.8rem;
text-align: center;
border:1px solid rgba(73,120,176,1);
...
.fcoursefabubtn_wrapheight{height:1.7rem;} 
</style>
